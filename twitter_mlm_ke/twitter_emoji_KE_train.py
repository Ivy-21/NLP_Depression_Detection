...
def train(model, train_loader, val_loader, num_epochs, optimizer, device):
    ...
    model.train()
    logger = Logger(model_name = 'twitter_emoji_KE', data_name = 'train_4')
    ...
    for epoch in range(num_epochs):
        ...
        val_logger = Logger(model_name = 'twitter_emoji_KE', data_name = 'train_4_val')
        val_start = time.time()
        with torch.no_grad():
            model.eval()
            for i, (batch) in enumerate(val_loader):
                ...
                val_end = time.time()
                val_logger.log(loss, val_acc, epoch, i, len(val_loader))

                if loss.item() < best_val_loss:
                    best_val_loss = loss.item()
                    best_val_acc = val_acc
                    logger.save_models(model, epoch, i)
                    # model.save_pretrained('./models/twitter_emoji_KE/train_3/')
